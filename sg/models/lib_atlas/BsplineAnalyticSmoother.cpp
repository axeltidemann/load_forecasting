...
BsplineAnalyticSmoother::BsplineAnalyticSmoother(double *dataset, unsigned int n_data, double *knots, unsigned int n_knot,  int degree, double smoothness, double zscore, int n_threads)
{
	//this->n_threads = sysconf( _SC_NPROCESSORS_ONLN );
	this->degree = degree;
...
/*	
	...
	cout << "in c(address):" << dataset << endl;
*/	
...
double BsplineAnalyticSmoother::bsplinebasis(unsigned int i, int p, double t)
{
	double this_range = 0;
    	...
    	if( i < 0 )
        	fprintf(stderr, "Error in i vale: %d\n", i);
...
*/
	...
	info = clapack_dgesv(CblasRowMajor, n_coef, n_coef, R, n_coef, ipiv, core, n_coef);
	//dgesv_(&n_coef, &n_coef, R, &n_coef, ipiv, core, &n_coef, &info);	
...
/*
	cout << "in caldataq(address):" << dataset << endl;

	cout << "Inside calc_cleanedData" << endl;
	for(int i=0; i<n_data; i++)
		cout << dataset[i] << endl;
*/
	...
	#pragma omp parallel for num_threads(n_threads)  private(diff, indx) reduction(+:total_se, df)
	for(unsigned int i=0; i < n_data; i++)
	{
	...
	#pragma omp parallel for num_threads(n_threads)  private(pe, lw, up)
	for(unsigned int i=0; i < n_data; i++)
	{
...
double* BsplineAnalyticSmoother::get_roughness()
{
	...
	// Parallel = OK
	#pragma omp parallel for num_threads(n_threads) private(indx, data) collapse(2)
...
extern "C" {
	BsplineAnalyticSmoother* Soother_new(double *dataset, unsigned int n_data, double *knots, unsigned int n_knot,  int degree, double smoothness, double zscore, int n_threads){ return new BsplineAnalyticSmoother(dataset, n_data, knots, n_knot, degree, smoothness, zscore, n_threads); }
